...
// กรองและ map ตัวที่ขึ้นต้นด้วยเลข 8
      ...
      .then((data) => {
        data.docs.forEach((search) => {
          fetch("https://openlibrary.org" + search.key + ".json")
            .then((response) => response.json())
            .then((bookData) => {
              const filteredIsbnList = search.isbn
                .filter((isbn) => isbn.startsWith("1"))
                .map((isbn) => isbn);

              console.log(filteredIsbnList);
              ...
              rating.innerText = span_rate.innerHTML + search.ratings_average;
            ...
            .catch((error) => {
              ...
              const filteredIsbnList = search.isbn
                .filter((isbn) => isbn.startsWith("1", "8"))
                .map((isbn) => isbn);
